...
php7.0 $APP maf:tor:update 2>&1 > $LOGDIR/exits-$DAY.log
php7.0 $APP maf:process:expires 2>&1 > $LOGDIR/turn-$DAY.log
php7.0 $APP maf:process:economy -t 2>&1 >> $LOGDIR/turn-$DAY.log
php7.0 $APP maf:run -t -d turn 2>&1 >> $LOGDIR/turn-$DAY.log
php7.0 $APP maf:process:convs 2>&1 >> $LOGDIR/turn-$DAY.log
php7.0 $APP maf:process:slumbers -t 2>&1 >> $LOGDIR/turn-$DAY.log
