...
class ProcessFamiliarityCommand extends AbstractProcessCommand {
	...
	protected function execute(InputInterface $input, OutputInterface $output): int {
		...
		$conn->executeStatement('UPDATE soldier set distance_home = ROUND(ST_Distance(c.location, gh.center)) FROM playercharacter c, settlement h, geodata gh WHERE soldier.character_id=c.id and soldier.home_id=h.id and h.geo_data_id = gh.id and c.travel is not null');
		$conn->executeStatement('UPDATE entourage set distance_home = ROUND(ST_Distance(c.location, gh.center)) FROM playercharacter c, settlement h, geodata gh WHERE entourage.character_id=c.id and entourage.home_id=h.id and h.geo_data_id = gh.id and c.travel is not null');
	...
	private function decayFamiliarity(): void {
		$this->em->createQuery('UPDATE App\Entity\RegionFamiliarity f SET f.amount=f.amount-1')->execute();
		$this->em->createQuery('DELETE FROM App\Entity\RegionFamiliarity f WHERE f.amount <= 0')->execute();
	}
