...
class Interactions {
	...
	public function characterLeaveSettlement(Character $character, $force = false): bool {
	...
	public function characterLeavePlace(Character $character, $force = false): bool {
		$place = $character->getInsidePlace();
		if (!$place) return false;

		if ($force && (!$place->isFortified() || $this->pm->checkPlacePermission($place, $character, 'visit'))) {
			// people with visiting permission cannot be forced out
			return false;
		}
	...
	public function abandonSettlement(Character $character, Settlement $settlement, $keepRealm): bool {
