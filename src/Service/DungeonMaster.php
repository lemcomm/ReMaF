...
class DungeonMaster {
	...
	private int $max_cards_per_type = 8;
	private int $starting_wounds = 10;
	private int $max_notify_distance = 50000; // this should be the same as the hardcoded value in CharacterController::summaryAction

	private int $total_cards = 0;

	private array $basicset = array(
		'basic.leave'		=> 1,
		'basic.rest'		=> 2,
		'basic.scout'		=> 3,
		'basic.plunder'	=> 4,
	...
	public function cleanupDungeoneer(Character $character): bool {
	...
	public function retireDungeoneer(Character $character): bool {
		$dungeoneer = $character->getDungeoneer();
		if (!$dungeoneer) return false;
	...
	private function randomAction(Dungeoneer $dungeoneer): void {
	...
	public function calculateTurnTime(Dungeon $dungeon): array {
		$missing = 0;
		$party = 0;
		if (!$dungeon->getParty()) {
...
In short before a dungeon starts you get a bit longer, but when it's running you get a while but not as long. --Andrew */
	...
	public function addEvent(DungeonParty $party, $content, $data=null): void {
	...
	private function Modifiers(DungeonParty $party): void {
	...
	private function Others(DungeonParty $party): void {
	...
	private function regainRandomAction(Dungeoneer $dungeoneer): void {
	...
	private function disarmTrap(Dungeoneer $dungeoneer, $chance): void {
	...
	private function Scouting(DungeonParty $party): void {
	...
	private function Fights(DungeonParty $party): void {
	...
	private function findMonsterTarget(Dungeoneer $dungeoneer): DungeonMonster {
	...
	private function DungeoneerAttack(Dungeoneer $dungeoneer, DungeonMonster $target, $damage=1, $powermod=1.0, $defmod=1.0): void {
	...
	private function MonsterAttack(DungeonMonster $monster, Dungeoneer $victim, $scout_level): void {
	...
	private function Plunder(DungeonParty $party): void {
	...
	private function resolvePlunder(Dungeoneer $dungeoneer, DungeonTreasure $treasure): void {
	...
	private function Leave(DungeonParty $party): void {
	...
	public function exitDungeon(Dungeoneer $hero, $goldmod, $cards): void {
	...
	public function createParty(Dungeon $dungeon): DungeonParty {
	...
	public function NewCard(Dungeoneer $hero, $notify = true): void {
	...
	public function leaveParty(Dungeoneer $hero): void {
	...
	public function dissolveParty(DungeonParty $party): void {
	...
	private function RemoveFromParty(Dungeoneer $member, DungeonParty $party): void {
	...
	private function Proceed(DungeonParty $party): void {
