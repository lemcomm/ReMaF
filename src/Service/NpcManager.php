...
class NpcManager {
	...
	protected array $mercenary_titles = array(
	...
	public function __construct(EntityManagerInterface $em, LoggerInterface $logger, Generator $generator, Geography $geo, History $history, CharacterManager $cm) {
		$this->em = $em;
		$this->logger = $logger;
	...
	private function setBaseValues(Character $npc): void {
	...
	public function spawnNPC(Character $npc): bool {
		...
		$weapons = $this->em->createQuery("SELECT COUNT(e) FROM App:EquipmentType e WHERE e.type = 'weapon' and e.resupply_cost <= 200")->getSingleScalarResult();
		$armours = $this->em->createQuery("SELECT COUNT(e) FROM App:EquipmentType e WHERE e.type = 'armour' and e.resupply_cost <= 200")->getSingleScalarResult();
		$items = $this->em->createQuery("SELECT COUNT(e) FROM App:EquipmentType e WHERE e.type = 'equipment' and e.resupply_cost <= 300")->getSingleScalarResult();
		$group = 1;

		while ($total_visual_size > 0) {
			$weapon = $this->em->createQuery("SELECT e FROM App:EquipmentType e WHERE e.type = 'weapon' and e.resupply_cost <= 200")
						->setFirstResult(rand(0, $weapons-1))->setMaxResults(1)->getSingleResult();
			if (rand(0,10)==0) {
				$armour = null;
			} else {
				$armour = $this->em->createQuery("SELECT e FROM App:EquipmentType e WHERE e.type = 'armour' and e.resupply_cost <= 200")
							->setFirstResult(rand(0, $armours-1))->setMaxResults(1)->getSingleResult();
			}
			if (rand(0,10)<6) {
				$item = null;
			} else {
				$item = $this->em->createQuery("SELECT e FROM App:EquipmentType e WHERE e.type = 'equipment' and e.resupply_cost <= 300")
	...
	public function checkTimeouts(Character $npc): void {
	...
	public function checkTroops(Character $npc): void {
