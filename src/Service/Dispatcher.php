...
class Dispatcher {
	...
	*/
	...
	public function PlacesActions(): array {
		...
		foreach ($this->geography->findPlacesInActionRange($this->getCharacter()) as $place) {
			$actions[] = array("title"=>$place->getFormalName());
			$actions[] = array("name"=>"place.view.name", "url"=>"maf_place_view", "parameters"=>array("id"=>$place->getId()), "description"=>"place.view.description", "long"=>"place.view.longdesc");
	...
	/*public function militaryAttackSettlementTest($check_duplicate=false) {
		...
		return $this->action("military.settlement.attack", "maf_war_attacksettlement");
	}
