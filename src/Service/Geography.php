...
class Geography {
	...
	public function findRegionFamiliarity(Character $character, GeoData $geo) {
		$fam = $this->em->getRepository(RegionFamiliarity::class)->findOneBy(['character'=>$character, 'geo_data'=>$geo]);
		if ($fam) {
	...
	public function findMyShip(Character $character) {
		$my_ship = $this->em->getRepository(Ship::class)->findOneByOwner($character);
		if (!$my_ship) {
	...
	public function checkTravelRivers(Character $character, $invalid) {
		...
		foreach ($result as $data) {
			...
			if ($bridge) {
				$bridges[] = ['river'=>$river->getName(), 'bridgename'=>$bridge[0]->getName(), 'bridgelocation'=>$bridge[0]->getGeoData()->getSettlement()->getName()];
			} else {
				$invalid[] = ['type'=>'feature', 'geometry'=>json_decode($data['json'])];
			}
		}
	...
	public function checkTravelRoads(Character $character) {
		...
		$result = $query->getResult();
		foreach ($result as $data) {
			$roads[] = ['type'=>'feature', 'geometry'=>json_decode($data['intersections'])];
		}
		if (empty($roads)) {
			$roads = false;
		} else {
