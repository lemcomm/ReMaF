	...
	public function checkSpecialConditions(Settlement $settlement, $building_name) {
		...
		switch (strtolower($building_name)) {
			...
			case 'fishery':	// only at ocean or lake
				if (!$settlement->getGeoData()->getCoast() && !$settlement->getGeoData()->getLake()) {
					return false;
				}
				break;
			case 'hunters lodge':
			...
			case 'imperial seat': // only if lord is ruler of realm
				...
				if ($settlement->getRealm()->isUltimate()) {
					...
					if (is_array($settlement->getCapitalOf()->toArray())) {
						...
						foreach ($settlement->getCapitalOf() as $cap) {
							if (in_array($cap->getId(), $realmids)) {
								$matchedrealms = true;
								break;
							}
						}
			...
			case 'filled moat': // only at a region that has water
				if (!$settlement->getGeoData()->getCoast() && !$settlement->getGeoData()->getLake() && !$settlement->getGeoData()->getRiver()) {
					return false;
				}
				break;
	...
	public function getSupplyTravelTime(Settlement $start, Unit $unit) {
		if ($unit->getDefendingSettlement()) {
			...
			if ($place->getSettlement()) {
				$distance = $this->geo->calculateDistanceBetweenSettlements($start, $place->getSettlement());
			} else {
	...
	public function ResourceDemand(Settlement $settlement, ResourceType $resource, $split_results=false) {
		...
		switch (strtolower($resource->getName())) {
			case 'food':
				...
				if ($suppliedNPCs > 0) {
					$suppliedNPCs = ceil($suppliedNPCs/2); #TODO: as funny as full effect would be :)
				}
