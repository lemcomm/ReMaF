...
class LoadBuildingData extends Fixture implements DependentFixtureInterface {

	private array $buildings = array(
		...
		'Mill'  		=> array('auto' =>    500, 'min' =>    200, 'work' =>  14000, 'ratio' =>  2500, 'builtin' => array('city'), 'icon'=>'buildings/mill.png'),
	...
	public function load(ObjectManager $manager): void {
		...
		foreach ($this->buildings as $name=>$data) {
			$type = $manager->getRepository(BuildingType::class)->findOneBy(['name'=>$name]);
			if (!$type) {
			...
			$type->setDefenses($data['defenses'] ?? 0);
			$type->setSpecialConditions(isset($data['conditions']));
			if (isset($data['icon'])) {
			...
			foreach ($this->resources[$name] as $resourcename => $resourcedata) {
				...
				$br->setRequiresConstruction($resourcedata['construction'] ?? 0);
				$br->setRequiresOperation($resourcedata['operation'] ?? 0);
				$br->setProvidesOperation($resourcedata['provides'] ?? 0);
				$br->setProvidesOperationBonus($resourcedata['bonus'] ?? 0);
			}
		}
		foreach ($this->buildings as $name=>$data) {
